    ...
    >
      ...
      <div ref={stage1OuterRef} className="relative" style={{ height: '170vh' }}>
        <div className="sticky top-0 h-screen w-full px-side-m md:px-side-t xl:px-side-d">
          ...
          <p
            data-cm-paragraph
            className="absolute left-1/2 max-w-[720px] text-center font-kr font-light text-body-l leading-relaxed text-ink-inverse/56"
            style={{
              top: '28vh',
              ...
              <span key={i} className="block">
                ...
                {line.split(/(\*\*[^*]+\*\*)/g).map((seg, j) =>
                  seg.startsWith('**') && seg.endsWith('**') ? (
                    <span key={j} className="font-semibold text-ink-inverse" style={{ fontSize: '1.06em' }}>
                      {seg.slice(2, -2)}
                    </span>
                  ) : (
                    <span key={j}>{seg}</span>
